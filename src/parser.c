...
layer parse_shortcut(list *options, size_params params, network net)
{
    ...
    layer s = make_shortcut_layer(params.batch, n, layers, sizes, params.w, params.h, params.c, layers_output,
                                  layers_delta,
                                  layers_output_gpu, layers_delta_gpu, weights_type, weights_normalizion, activation,
                                  params.train);
...
void parse_net_options(list *options, network *net)
{
    ...
    net->time_steps = option_find_int_quiet(options, "time_steps", 1);  // used in rnn, ignore
...
// 配置文件中第一个section必须是[net]或[network]
...
network parse_network_cfg_custom(char *filename, int batch, int time_steps)
{
    ...
    network net = make_network(sections->size - 1);  // 创建network对象并初始化
    net.gpu_index = gpu_index;  // extern, declared in darknet.h, assign value in darknet.c main()
    size_params params;

    if (batch > 0) params.train = 0;    // allocates memory for Detection only
    else params.train = 1;              // allocates memory for Detection & Training

    section *s = (section *) n->val;    // [net]
    list *options = s->options;         // options in [net]
    if (!is_network(s)) error("First section must be [net] or [network]");
    parse_net_options(options, &net);   // 解析[net]中的配置，为network结构体赋值
...
// 读取.cfg配置文件，每个[xxx]结构为一个section，最后返回由所有section组成的链表
list *read_cfg(char *filename)  //yolov4.cfg
{

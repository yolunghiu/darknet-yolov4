                    ...
                    int calc_map, int mjpeg_port, int show_imgs, int benchmark_layers, char *chart_path)
{
    ...
    // 若使用了`-map`参数，则创建用于计算map的网络
    network net_map;
    if (calc_map)
    {
        FILE *valid_file = fopen(valid_images, "r");  // 将2007_test_test.txt中的图片用于map计算
        if (!valid_file)
        {
    ...
    char *base = basecfg(cfgfile);  // yolov4-voc
    printf("%s\n", base);
    float avg_loss = -1;
    ...
    int imgs = net.batch * net.subdivisions * ngpus;    // 一个batch中所有gpu共处理的图片数
    ...
    iter_map = get_current_iteration(net);          // 分别是：上一次保存模型时的迭代次数、最后一次保存模型时的迭代次数、map模型迭代次数
    ...
    net.train_images_num = train_images_num;    // equal to args.m
    args.d = &buffer;
    ...
    if (net.track)  // track参数从配置文件中读取，默认值为0
    {
    ...
    while (get_current_iteration(net) < net.max_batches)
    {
        ...
        if (l.random && count++ % 10 == 0)
        {
            ...
            // 随机改变尺度后输入图片的尺寸, 要能被resize_step(32)整除
            ...
            // 多线程读取图片, 由于输入图片的宽高（以及batch size）改变了，需要重新读取数据
        ...
        pthread_join(load_thread, 0);   // 运行load_thread()函数, 这里包含了数据增强方法
        train = buffer;
        if (net.track)
        {
                   ...
                   int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    ...
    while (1)
    {
        ...
        if (!dont_show)
        {
            show_image(im, "predictions");  // need opencv support
        }
...
{
    // 经过下面多个find操作之后，使用[-key value | -key]指定的参数在取出值后都从argv中移除了，argv中只保留了必须指定的参数
    ...
    if (gpu_list)
    {
        ...
        for (i = 0; i < ngpus; ++i)
        {
            ...
            gpu_list = strchr(gpu_list, ',') + 1;  // 找到第一次出现指定字符的位置
        }
    } else
    {
        gpu = gpu_index;
    ...
    char *filename = (argc > 6) ? argv[6] : 0;  // image path
    ...
    } else if (0 == strcmp(argv[2], "demo"))
    {
        ...
        demo(cfg, weights, thresh, hier_thresh, cam_index, filename, names, classes, frame_skip, prefix, out_filename,
             mjpeg_port, json_port, dont_show, ext_output, letter_box, time_limit_sec, http_post_host, benchmark,
             benchmark_layers);
